...
impl Hub {
    ...
    pub fn join(&self, request: JoinRequest) -> Result<bool, HubError> {
        ...
        match request.role {
            ...
            Role::App => {
                ...
                if let Some(old_app) = evicted {
                    ...
                    // so it can remain idle.
                    let _ = old_app.outbound.try_send(OuterFrame::Evicted);
                    app_evicted = true;
                }
            }
        }
        ...
        if let Some(peer) = peer {
            // Tell the already-present peer that the newcomer joined.
            let _ = peer.outbound.try_send(OuterFrame::PeerJoined {
                role: request.role,
                device_pubkey: request.device_pubkey,
                pairing_token_proof: request.pairing_token_proof,
                connection_salt: request.connection_salt,
            ...
            let _ = request.outbound.try_send(OuterFrame::PeerJoined {
                role: peer_role,
    ...
    pub fn leave(&self, room_id: &str, role: Role, conn_id: ConnId) {
        if let Some(mut room) = self.rooms.get_mut(room_id) {
            if room.matches(role, conn_id) {
                ...
                if let Some(peer) = peer {
                    let _ = peer.outbound.try_send(OuterFrame::PeerLeft { role });
                }
            }
        }
