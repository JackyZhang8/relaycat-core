...
fn collect_row_attr_ids<'a>(rows: impl Iterator<Item = &'a TerminalRow>, used: &mut BTreeSet<u32>) {
    for row in rows {
        for run in &row.cells {
            used.insert(run.attr_id);
        }
    }
}

pub(crate) fn remap_row_attr_ids<'a>(
    rows: impl Iterator<Item = &'a mut TerminalRow>,
    remap: &HashMap<u32, u32>,
) {
    for row in rows {
        for run in &mut row.cells {
            run.attr_id = remap.get(&run.attr_id).copied().unwrap_or(0);
        }
    }
}
...
impl TerminalCore {
    ...
    /// Garbage-collect the append-only attribute table, keeping only entries
    /// still referenced by retained rows and remapping every stored `attr_id`.
    ///
    /// Only called right before emitting a fresh full snapshot (which resets
    /// the app's attribute table), so remapped ids never mix with the table a
    /// connected app already holds. Returns the old-id -> new-id map, or
    /// `None` when nothing could be reclaimed.
    pub(crate) fn compact_attrs(&mut self) -> Option<HashMap<u32, u32>> {
        let mut used = BTreeSet::new();
        used.insert(0_u32);
        collect_row_attr_ids(self.history.iter(), &mut used);
        collect_row_attr_ids(self.pending_scrollback_append.iter(), &mut used);
        collect_row_attr_ids(self.screen_rows.iter(), &mut used);
        for entry in &self.transcript_store.entries {
            collect_row_attr_ids(entry.rows.iter(), &mut used);
        }
        used.retain(|id| usize::try_from(*id).is_ok_and(|id| id < self.attrs.len()));
        if used.len() >= self.attrs.len() {
            return None;
        }

        let mut remap = HashMap::with_capacity(used.len());
        let mut new_attrs = Vec::with_capacity(used.len());
        for old_id in used {
            let new_id = u32::try_from(new_attrs.len()).expect("attr table index overflow");
            new_attrs.push(self.attrs[old_id as usize].clone());
            remap.insert(old_id, new_id);
        }

        remap_row_attr_ids(self.history.iter_mut(), &remap);
        remap_row_attr_ids(self.pending_scrollback_append.iter_mut(), &remap);
        remap_row_attr_ids(self.screen_rows.iter_mut(), &remap);
        for entry in &mut self.transcript_store.entries {
            remap_row_attr_ids(entry.rows.iter_mut(), &remap);
        }
        ...
        // table; they can no longer be replayed for resume.
        self.retained_patches.clear();
        self.emitted_attrs_len = 0;
        Some(remap)
    }
