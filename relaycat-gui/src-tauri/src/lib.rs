...
fn create_relay_session(
    ...
    let pairing_url_path =
        relaycat_cli::pairing_store::gui_pairing_url_path(&project_dir, &target.session_kind);
    ...
    let resize_file = project_dir
        ...
        .join(format!("gui-resize-{}.txt", target.session_kind.as_str()));
