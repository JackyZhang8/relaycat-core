...
impl SessionManager {
    ...
    fn spawn_inner(
        ...
        let session = PtySession::spawn(target, rows, cols)?;
    ...
    fn wire_session(
        ...
        if let Some(relay) = relay {
            spawn_log_tail(app.clone(), id.clone(), relay.log_path, alive.clone());
...
fn spawn_log_tail(app: AppHandle, id: String, log_path: PathBuf, alive: Arc<AtomicBool>) {
    thread::spawn(move || {
        ...
        while alive.load(Ordering::Acquire) {
            ...
            match reader.read_line(&mut line) {
                ...
                Ok(_) => {
                    if !paired && line.contains("secure session established") {
                        paired = true;
                        let _ = app.emit(
                            "session://relay",
                            RelayEvent {
